$(function () {
    ...
    $('.Header .top_service .lang strong').on('click', function () {
        $(this).toggleClass('on');
        $('.Header .top_service .lang').toggleClass('on');
    })

    $('.top_serch strong').on('click', function () {
        $(this).toggleClass('on')
        $('.top_search').toggleClass('on')
    })

    $('.movie_case').YTPlayer({
        videoURL: 'https://youtu.be/raw3Nu0_mBQ', containment: 'self', autoPlay: true, mute: true, showControls: false, playOnlyIfVisible: true,
    });
    ...
    $('.Footer .ft_top .right li').on('click', function (e) {
        e.preventDefault()
        var idx = $(this).index()
        console.log(idx)
        if ($(this).hasClass('on')) {
